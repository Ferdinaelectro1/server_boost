...
{
    _user_client_handle = std::make_shared<std::function<void()>>(func);
}
...
void Server::_restart_acceptor()
{
    _acceptor.async_accept([&](const boost::system::error_code& error, tcp::socket socket){
            ...
            if(_user_client_handle != nullptr)
            {
                _sessions[_id]->user_handle = *_user_client_handle;     
            } 
            _restart_acceptor();
            _id++;
    });
